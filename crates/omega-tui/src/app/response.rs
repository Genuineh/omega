...
impl App {
    ...
    /// T-60: jump the response cursor to the latest turn (i.e.
    /// the bottom of the response panel). Sets
    /// `response_turn_index = 0` (auto-follow) and selects the
    /// last line.
    pub fn jump_response_to_latest_turn(&mut self) {
        let lines = self.response_display_lines();
        if lines.is_empty() {
            return;
        }
        self.response_pinned = false;
        self.response_turn_index = 0; // 0 = follow latest
        self.response_state.select(Some(lines.len().saturating_sub(1)));
    }

    /// T-60: jump the response cursor to the oldest turn.
    /// Sets `response_turn_index = response_turn_count` and
    /// selects the first line.
    pub fn jump_response_to_oldest_turn(&mut self) {
        let lines = self.response_display_lines();
        if lines.is_empty() {
            return;
        }
        self.response_pinned = true;
        self.response_turn_index = self.response_turn_count.max(1);
        self.response_state.select(Some(0));
    }

    /// T-60: jump to the next turn (or wrap around to the
    /// earliest). Returns `true` if the cursor actually moved.
    pub fn jump_response_to_next_turn(&mut self) -> bool {
        let lines = self.response_display_lines();
        if lines.is_empty() {
            return false;
        }
        let current = self.response_state.selected().unwrap_or(0).min(lines.len() - 1);
        // Find the start of the next turn (a User line) after
        // `current`. If none, wrap to the earliest turn.
        let next = lines
            .iter()
            .enumerate()
            .skip(current + 1)
            .find(|(_, l)| l.kind == MsgKind::User)
            .map(|(i, _)| i)
            .or_else(|| {
                // Wrap to the first User line.
                lines.iter().position(|l| l.kind == MsgKind::User)
            });
        if let Some(idx) = next {
            self.response_pinned = true;
            self.response_turn_index = self.response_turn_count.saturating_sub(1);
            self.response_state.select(Some(idx));
            true
        } else {
            false
        }
    ...
    /// T-60: jump to the previous turn. Returns `true` if the
    /// cursor actually moved.
    pub fn jump_response_to_prev_turn(&mut self) -> bool {
        let lines = self.response_display_lines();
        if lines.is_empty() {
            return false;
        }
        let current = self.response_state.selected().unwrap_or(0).min(lines.len() - 1);
        // Find the most recent User line BEFORE `current`.
        let prev = (0..current)
            .rev()
            .find(|&i| lines[i].kind == MsgKind::User);
    ...
    pub fn activate_response_item_at_line(
        ...
        // T-57: when the line carries an explicit action, use it;
        // otherwise, if it's a section header (is_header == true)
        // for a section kind, open the StepDetailOverlay.
        if let Some(action) = line.action.clone() {
            return self.activate_response_action(action);
        }
        // T-61 / T-71: pressing Enter on a User line OR a
        // body-only line (Error / Agent) opens the
        // TurnDetailOverlay for the turn that contains this
        // line. The popup is the source of truth for the
        // full body (e.g. the multi-line stack trace / HTTP
        // 400 response for an Error record), so users
        // clicking on an `✗ Hook-managed step failed: ...`
        // row in the chat log can read the full error in
        // the popup.
        if line.kind == MsgKind::User
            || matches!(line.kind, MsgKind::Error | MsgKind::Agent)
        {
            // Find which turn this line belongs to. Use
            // `iter_turns_with_offsets` so we can map the
            // source-line index back to a turn.
            use crate::render::chat_turn::iter_turns_with_offsets;
            let turns_with_offsets = iter_turns_with_offsets(&lines);
            for (i, tw) in turns_with_offsets.iter().enumerate() {
                let turn = &tw.turn;
                let source_offset = tw.source_offset;
                if line.kind == MsgKind::User {
                    // The user line is at `source_offset`.
                    let user_match = turn
                        .user_msg
                        .message_id
                        .as_deref()
                        .map(|id| line.message_id.as_deref() == Some(id))
                        .unwrap_or(false)
                        || turn.user_msg.text == line.text;
                    if user_match {
                        return self
                            .open_turn_detail_overlay(i + 1)
                            .map(|_| ResponseActivation::TurnDetailOpened(0));
                    }
                } else {
                    // For body lines (Error / Agent), the
                    // source line index of the j-th agent
                    // msg is `source_offset + 1 + j`.
                    for (j, agent) in turn.agent_msgs.iter().enumerate() {
                        let agent_source_index = source_offset + 1 + j;
                        if agent_source_index == line_index {
                            return self
                                .open_turn_detail_overlay(i + 1)
                                .map(|_| ResponseActivation::TurnDetailOpened(0));
                        }
        ...
        if line.is_header {
            if let Some(section_id) = line.message_id.clone() {
                let title = if line.text.is_empty() {
                    "Step".to_string()
                } else {
                    line.text.clone()
                };
                return self
                    .open_step_detail_overlay(&section_id, title)
                    .map(|_| ResponseActivation::StepDetailOpened(section_id));
            }
        }
        None
    }
    ...
    fn activate_response_action(
        ...
        match action {
            ...
            ResponseLineAction::OpenStepDetail(id, _title) => self
                .open_step_detail_overlay(&id, _title)
                .map(ResponseActivation::StepDetailOpened),
        }
    }

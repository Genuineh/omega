...
impl ProjectPlanStore {
	...
	pub fn mutate_dependency(
		...
		match operation {
			TaskDependencyOperation::Add => {
				if !task.depends_on.iter().any(|dependency| dependency == other_id) {
					if self.dependency_reaches(other_id, task_id)? {
						anyhow::bail!("adding dependency {task_id} -> {other_id} would create a cycle");
					}
	...
	pub fn add_artifact_link(
		...
		if !links.iter().any(|existing| existing.path == link.path && existing.kind == link.kind) {
		...
		self.append_log(
			...
			TaskLogKind::NoteAdded,
			TaskActor::Command,
			format!("Linked {} artifact {}", match surface { TaskLinkSurface::Design => "design", TaskLinkSurface::Implementation => "implementation" }, link.path),
			None,
			None,
			None,
		)?;
	...
	pub fn load_logs(&self, task_id: &str, limit: Option<usize>) -> Result<Vec<TaskLogEntry>> {
		...
		let mut entries = BufReader::new(file)
			...
			.map(|line| -> Result<Option<TaskLogEntry>> {
				...
				Ok(Some(serde_json::from_str::<TaskLogEntry>(&line).with_context(
	...
	fn load_tasks(&self) -> Result<Vec<PlannedTask>> {
		...
		let mut tasks = BufReader::new(file)
			...
			.map(|line| -> Result<Option<PlannedTask>> {
				...
				Ok(Some(serde_json::from_str::<PlannedTask>(&line).with_context(
	...
	fn list_tasks(&self, filter: TaskListFilter) -> Result<Vec<PlannedTask>> {
		...
		tasks.retain(|task| {
			filter.status.is_none_or(|status| status == task.status)
				&& filter.priority.is_none_or(|priority| priority == task.priority)
		});
		tasks.sort_by(compare_tasks);
		Ok(tasks)
	}
...
mod tests {
	...
	use super::{
		NewPlannedTask, PlannedTaskStatus, PlannedTaskUpdate, ProjectPlanAccess,
		ProjectPlanStore, TaskActor, TaskArtifactKind, TaskArtifactLink,
		TaskDependencyOperation, TaskLinkSurface, TaskListFilter, TaskLogKind,
	...
	fn open_or_scaffold_creates_manifest_and_task_dirs() {
		...
		assert!(store.list_tasks(TaskListFilter::default()).unwrap().is_empty());
	}
	...
	fn resolve_task_context_includes_dependencies_and_recent_logs() {
		...
		assert_eq!(context.dependency_chain, vec![format!("{}: Dependency", dependency.id)]);

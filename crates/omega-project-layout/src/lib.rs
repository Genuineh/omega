...
impl OmegaProjectLayout {
    ...
    pub fn legacy_session_record_path(&self, session_id: &str) -> PathBuf {
        self.legacy_sessions_dir().join(format!("{session_id}.json"))
    }
...
mod tests {
    ...
    fn resolves_config_and_state_roots_separately() {
        ...
        assert_eq!(layout.project_manifest_path(), root.join(".omega/project.toml"));
        assert_eq!(layout.project_state_path(), root.join(".omega-state/project.json"));
        assert_eq!(layout.legacy_project_state_path(), root.join(".omega/project.json"));
    }

...
pub use omega_hpc_document::{SharedTodoManager, TodoItem, TodoManager, TodoStatus, TODO_REMINDER};
...
mod tests {
    ...
    fn todo_read_returns_rendered_snapshot() {
        ...
        let output = TodoReadHandler::with_manager(shared)
            .execute(json!({}))
            .unwrap();
    ...
    fn todo_write_updates_shared_manager() {
        ...
        assert_eq!(
            shared.lock().unwrap().items()[0].status,

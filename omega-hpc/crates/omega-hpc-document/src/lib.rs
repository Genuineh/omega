...
pub use structured_docs::{
    ...
    StructuredDocsRenderState, StructuredDocsSnapshot, StructuredDocsValidationIssue,
    StructuredDocsValidationReport, StructuredDocumentRecord, StructuredDocumentRelation,
    StructuredDocumentRender, StructuredDocumentSection, TaskProjection,
};
use structured_docs::{StructuredDocsManager, StructuredDocumentOpOutcome};
pub use todo::{
    SharedTodoManager, TodoItem, TodoManager, TodoStatus, MAX_TODO_ITEMS, TODO_REMINDER,
};
...
impl OmegaDocument {
                ...
                Ok(()) => {
                    ...
                    if let Some(previous_active) = previous_ledger.active_version.as_ref() {
                        archived_version_path =
                            self.file_store.archive_active_version(previous_active)?;
                    }
    ...
    pub fn manage_document(&self, op: DocumentOp) -> Result<DocumentOpResult> {
        match op {
            ...
            DocumentOp::UpsertRecord { mode, record } => self.structured_result(
                Some(mode),
                self.structured_docs().upsert_record(mode, record)?,
            ),
            ...
            DocumentOp::ArchiveRecord {
                ...
                Some(mode),
                self.structured_docs().archive_record(
                    mode,
                    &doc_id,
                    reason,
            ...
            DocumentOp::DeleteRelation { mode, relation_id } => self.structured_result(
                Some(mode),
            ...
            DocumentOp::ValidateProjection { doc_ids } => {
                self.structured_result(None, self.structured_docs().validate_projection(doc_ids)?)
            }
            DocumentOp::ExtractSource {
                ...
                self.structured_docs()
                    .extract_sources(mode, sources, doc_type)?,
            ),
        }
    }
    ...
    fn structured_result(
        ...
        let ok = outcome.validation.as_ref().is_none_or(|report| report.ok);
        Ok(DocumentOpResult {
            mode,
            ok,
    ...
    fn create_document(
        ...
        let missing_fields =
            find_missing_frontmatter_fields(content, &rules.lifecycle.required_frontmatter);
        for field in &missing_fields {
            validation_issues.push(format!(
                "content is missing required frontmatter field '{field}'"
            ));
        }
    ...
    fn archive_active_version(&self, version: &DocumentStoreVersion) -> Result<Option<String>> {
        ...
        if archived.root.exists() {
            fs::remove_dir_all(&archived.root).with_context(|| {
                format!(
                    "failed to replace archived store {}",
...
impl VectorIndex {
    ...
    fn rebuild_at(
        ...
        fs::create_dir_all(db_dir)
            .with_context(|| format!("failed to create vector index dir {}", db_dir.display()))?;
...
/// Path components that should never enter the document store, regardless of
/// where they appear in the tree. Matched at any depth so that sub-workspace
/// `target/` directories (`omega-hpc/target/`), nested `.omega-state/`
/// state dirs (`omega-hpc/.omega-state/...`), nested nix `result/`
/// symlinks, and per-crate `.git/` worktrees are all skipped without each
/// caller having to grow the ignore list.
const HARD_EXCLUDED_PATH_COMPONENTS: &[&str] = &[".git", "target", ".omega-state", "result", "node_modules"];

/// The `.omega-state/store/` directory itself is excluded by component
/// matching (`.omega-state` + `store`), but we also keep the explicit
/// `STORE_DIR` constant-based check so the rule reads as a single named
/// intent — the component check is the safety net for any future sub-path
/// under `.omega-state/store/...` that the constants don't enumerate.
const HARD_EXCLUDED_PATH_PREFIXES: &[&str] = &[STORE_DIR];
...
fn should_visit_path(root: &Path, path: &Path) -> bool {
    ...
    if HARD_EXCLUDED_PATH_PREFIXES
        .iter()
    ...
    for component in normalized.split('/') {
        if HARD_EXCLUDED_PATH_COMPONENTS.contains(&component) {
            return false;
        }
    }
    true
    ...
    fn load(root: &Path) -> Result<Self> {
        ...
        let matcher = if pattern_count == 0 {
            ...
            Some(
                builder
                    .build()
                    .context("failed to compile .storeignore rules")?,
            )
        };
...
mod tests {
    ...
    use super::{
        should_visit_path, ArchiveTrigger, DocType, DocumentMutationMode, DocumentOp, FileStatus,
        OmegaDocument, SearchMode, SearchQuery, StructuredDocumentRecord,
        StructuredDocumentRelation, StructuredDocumentRender, StructuredDocumentSection, TodoOp,
    };
    use crate::{TodoItem, TodoStatus};
    use omega_hpc_paths::{STORE_DIR_PATH, STORE_MANIFEST_PATH, STORE_VERSION_PATH};
    ...
    fn scan_workspace_excludes_storeignored_files_from_manifest() {
        ...
        assert_eq!(
            scan.vector_ignored_paths,
            vec!["docs/specs/ignored.md".to_string()]
        );
    ...
    #[test]
    fn should_visit_path_excludes_nested_target_and_state_at_any_depth() {
        let root = std::path::Path::new("/repo");

        // Always-visit: the root itself and real source files.
        assert!(should_visit_path(root, std::path::Path::new("/repo")));
        assert!(should_visit_path(
            root,
            std::path::Path::new("/repo/README.md")
        ));
        assert!(should_visit_path(
            root,
            std::path::Path::new("/repo/crates/demo/src/lib.rs")
        ));
        assert!(should_visit_path(
            root,
            std::path::Path::new("/repo/omega-hpc/crates/omega-hpc-document/src/lib.rs")
        ));

        // Top-level excluded components (legacy behaviour, still required).
        assert!(!should_visit_path(
            root,
            std::path::Path::new("/repo/target/debug/foo")
        ));
        assert!(!should_visit_path(
            root,
        ...
        // Nested sub-workspace: the actual omega-hpc regression case.
        assert!(!should_visit_path(
            root,
            std::path::Path::new("/repo/omega-hpc/target/debug/deps/foo.rmeta")
        ));
        assert!(!should_visit_path(
            root,
            std::path::Path::new("/repo/omega-hpc/.omega-state/store/files.jsonl")
        ));

        // Deeper nesting — a hypothetical sub-sub-workspace should also be
        // filtered, not just the immediate child.
        assert!(!should_visit_path(
            root,
            std::path::Path::new("/repo/experiments/bench/target/release/bar")
        ));
        assert!(!should_visit_path(
            root,
            std::path::Path::new("/repo/a/b/c/.git/HEAD")
        ));
        assert!(!should_visit_path(
            root,
            std::path::Path::new("/repo/x/y/z/.omega-state/store/manifest.json")
        ));

        // nix `result` symlinks and node_modules are also skipped at any depth.
        assert!(!should_visit_path(
            root,
            std::path::Path::new("/repo/result/bin/omega-doc")
        ));
        assert!(!should_visit_path(
            root,
            std::path::Path::new("/repo/some/deep/node_modules/lodash/index.js")
        ));
    }

    #[test]
    fn scan_workspace_excludes_nested_target_and_omega_state_via_should_visit() {
        // Regression: pre-hpc-extraction, `target/` and `.omega-state/store/`
        // were only filtered at the repo root. After extracting omega-memory
        // / omega-document / omega-doc-cli / omega-project-layout into
        // `omega-hpc/`, the sub-workspace's own `target/` and `.omega-state/`
        // directories need the same filter applied at any depth.
        let root = temp_root("nested-target-exclude");
        seed_repo(&root);

        // Sub-workspace pretending to be omega-hpc/.
        std::fs::create_dir_all(root.join("omega-hpc/target/debug/deps")).unwrap();
        std::fs::write(
            root.join("omega-hpc/target/debug/deps/large.rmeta"),
            "fake build artifact — must not be scanned",
        )
        .unwrap();
        std::fs::write(
            root.join("omega-hpc/target/debug/build-script-output.txt"),
            "fake build log",
        )
        .unwrap();

        std::fs::create_dir_all(root.join("omega-hpc/.omega-state/store")).unwrap();
        std::fs::write(
            root.join("omega-hpc/.omega-state/store/files.jsonl"),
            "{}\n",
        )
        .unwrap();
        std::fs::write(
            root.join("omega-hpc/.omega-state/store/manifest.json"),
            "{}\n",
        )
        .unwrap();

        // nix `result` symlink-style dir.
        std::fs::create_dir_all(root.join("result/bin")).unwrap();
        std::fs::write(root.join("result/bin/omega-doc"), "fake binary").unwrap();

        // Even deeper nesting — experiments/bench/ has its own target/.
        std::fs::create_dir_all(root.join("experiments/bench/target/release")).unwrap();
        std::fs::write(
            root.join("experiments/bench/target/release/bench"),
            "fake bench binary",
        )
        .unwrap();

        // Empty .storeignore — we want to prove the exclusion comes from
        // should_visit_path alone, not from the .storeignore config file.
        write_storeignore(&root, "");
        ...
        // The real source files from seed_repo() are still indexed.
        assert!(records.contains_key("docs/specs/example.md"));
        assert!(records.contains_key("crates/demo/src/lib.rs"));
        assert!(records.contains_key("README.md"));

        // The nested build/state/result/nested-target noise is gone.
        assert!(!records.contains_key("omega-hpc/target/debug/deps/large.rmeta"));
        assert!(!records.contains_key("omega-hpc/target/debug/build-script-output.txt"));
        assert!(!records.contains_key("omega-hpc/.omega-state/store/files.jsonl"));
        assert!(!records.contains_key("omega-hpc/.omega-state/store/manifest.json"));
        assert!(!records.contains_key("result/bin/omega-doc"));
        assert!(!records.contains_key("experiments/bench/target/release/bench"));

        // should_visit_path is a filter_entry callback, so it short-circuits
        // the WalkDir traversal before the inner per-file loop runs. The
        // storeignore-based counter therefore stays at 0 — the exclusion is
        // structural, not configured. (This is the regression we're
        // protecting against: pre-fix, the inner loop would have visited
        // and tried to blake3-hash the entire omega-hpc/target subtree.)
        assert_eq!(
            scan.vector_ignored_files, 0,
            "nested target/.omega-state/result/ exclusions should come from \
             should_visit_path (WalkDir filter_entry), not from .storeignore, \
             so vector_ignored_files stays at 0 by design"
        );
    }
    ...
    fn archive_record_moves_record_into_archive_projection() {
        ...
        assert_eq!(
            archive.records[0].render.presentation_path,
            "docs/archive/archive-me.md"
        );
        assert!(!root.join("docs/specs/archive-me.md").exists());
    ...
    fn todo_projection_renders_project_tasks_with_todo_scope() {
        ...
        use crate::structured_docs::TaskProjection;

        let tasks_dir = root.join("docs-data/tasks");
        std::fs::create_dir_all(&tasks_dir).unwrap();
        let visible = TaskProjection {
            id: "TASK-9001".to_string(),
            title: "Visible project task".to_string(),
            status: "ready".to_string(),
            priority: "p1".to_string(),
            doc_scope: vec!["todo".to_string()],
            summary: "Should appear in TODO".to_string(),
            depends_on: Vec::new(),
        };
        let hidden = TaskProjection {
            id: "TASK-9002".to_string(),
            title: "Hidden project task".to_string(),
            status: "ready".to_string(),
            priority: "p1".to_string(),
            doc_scope: Vec::new(),
            summary: "Should stay out of TODO".to_string(),
            depends_on: Vec::new(),
        };
        let jsonl = format!(
            "{}\n{}\n",
        ...
        std::fs::write(tasks_dir.join("project-tasks.jsonl"), jsonl).unwrap();
    ...
    fn validate_projection_reports_missing_generated_file() {
        ...
        assert_eq!(
            ...
            vec!["docs/guide/missing-generated.md".to_string()]
        );
    }
    ...
    fn validate_projection_reports_unregistered_files() {
        ...
        assert!(
            report
                .unregistered_files
                .contains(&"docs/guide/orphan.md".to_string()),
    ...
    fn validate_projection_accepts_relative_relation_targets() {
        ...
        let extract = documents
            .manage_document(DocumentOp::ExtractSource {
                ...
                sources: vec![
                    "docs/README.md".to_string(),
                    "docs/guide/omega-dev-guide.md".to_string(),
                ],
                doc_type: None,
            })
            .unwrap();
    ...
    fn validate_projection_accepts_repo_relative_targets_from_nested_docs() {
        ...
        let extract = documents
            .manage_document(DocumentOp::ExtractSource {
                mode: DocumentMutationMode::Apply,
                sources: vec![
                    "docs/specs/parent.md".to_string(),
                    "docs/specs/example.md".to_string(),
                ],
                doc_type: None,
            })
            .unwrap();

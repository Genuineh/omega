...
impl StructuredDocsManager {
    ...
    pub(crate) fn render_projection(
        ...
        let task_projection = self.load_task_projection().unwrap_or_default();
    ...
    pub(crate) fn validate_projection(
        ...
        if doc_ids.is_empty() {
            ...
            for path in &unregistered {
                report
                    .warnings
                    .push(format!("unregistered file in docs/: {path}"));
            }
        ...
        Ok(StructuredDocumentOpOutcome {
            message: if report.ok && warn_count == 0 {
                format!(
                    "validated structured docs projection for {} document(s)",
                    records.len()
                )
            } else if report.ok {
                format!(
                ...
                format!("structured docs projection has {} issue(s)", issue_count)
            },
            records,
    ...
    fn ensure_layout(&self) -> Result<()> {
        fs::create_dir_all(self.layout.docs_data_records_dir()).with_context(|| {
            format!(
                "failed to create {}",
                self.layout.docs_data_records_dir().display()
            )
        })?;
        ...
        fs::create_dir_all(self.layout.docs_data_relations_dir()).with_context(|| {
            format!(
                "failed to create {}",
                self.layout.docs_data_relations_dir().display()
            )
        })?;
        fs::create_dir_all(self.layout.docs_data_render_dir()).with_context(|| {
            format!(
                "failed to create {}",
                self.layout.docs_data_render_dir().display()
            )
        })?;
    ...
    fn load_manifest(&self) -> Result<StructuredDocsManifest> {
        ...
        let content =
            fs::read_to_string(self.layout.docs_data_manifest_path()).with_context(|| {
                format!(
                    "failed to read {}",
    ...
    fn load_render_state(&self) -> Result<StructuredDocsRenderState> {
        ...
        let content =
            fs::read_to_string(self.layout.docs_data_render_state_path()).with_context(|| {
                format!(
                    "failed to read {}",
    ...
    fn load_task_projection(&self) -> Result<Vec<TaskProjection>> {
        let path = self.root.join(&self.layout.docs_data_project_tasks_path());
        if !path.exists() {
            return Ok(Vec::new());
        }
        load_jsonl(&path)
...
fn render_document(
    ...
    if record.doc_type == DocType::Todo {
        let todo_tag = doc_type_tag(DocType::Todo);
        let mut open: Vec<&TaskProjection> = task_projection
            .iter()
            .filter(|t| {
                ...
                status != "done"
                    ...
                    && t.doc_scope
                        .iter()
                        .any(|scope| scope.eq_ignore_ascii_case(todo_tag))
            })
            .collect();
        if !open.is_empty() {
            open.sort_by(|a, b| a.priority.cmp(&b.priority).then(a.id.cmp(&b.id)));
            output.push('\n');
            output.push_str("## Project Tasks\n\n");
            for task in &open {
                let status_label = {
                    let s = task.status.replace('_', " ");
...
fn projection_version_for_render(
    ...
    StructuredDocVersionInfo {
        ...
        generation_id: manifest.last_generation_id.clone().unwrap_or_else(|| {
            format_generation_id(manifest.projection_version, manifest.content_revision)
        }),
    }
}
...
fn validate_render_state_versions(
    ...
    if rendered_revision != manifest.content_revision {
        return Some(StructuredDocsValidationIssue {
            path: manifest.render_state_path.clone(),
            message: "render state content_revision does not match manifest content_revision"
                .to_string(),
            expected_preview: Some(manifest.content_revision.to_string()),
    ...
    if rendered_projection != manifest.projection_version {
        return Some(StructuredDocsValidationIssue {
            path: manifest.render_state_path.clone(),
            message: "render state projection_version does not match manifest projection_version"
                .to_string(),
            expected_preview: Some(manifest.projection_version.to_string()),
    ...
    if Some(rendered_generation.clone()) != manifest.last_generation_id {
        return Some(StructuredDocsValidationIssue {
            path: manifest.render_state_path.clone(),
            message: "render state generation_id does not match manifest last_generation_id"
                .to_string(),
            expected_preview: manifest.last_generation_id.clone(),
...
fn validate_generated_doc_version(
    ...
    if actual_content_revision != Some(expected_version.content_revision) {
        return Ok(Some(StructuredDocsValidationIssue {
            path: path.to_string(),
            message: "generated doc content_revision does not match current canonical revision"
                .to_string(),
    ...
    if actual_projection_version != Some(expected_version.projection_version) {
        return Ok(Some(StructuredDocsValidationIssue {
            path: path.to_string(),
            message: "generated doc projection_version does not match current projection version"
                .to_string(),
        ...
        Value::Array(values) => {
            ...
            for item in values {
                match item {
                    ...
                    _ => output.push_str(&format!("{padding}  - {}\n", render_scalar(item))),
                }
            }
        }
        Value::Object(map) => {
...
fn push_section(sections: &mut Vec<StructuredDocumentSection>, heading: &str, body: &str) {

...
impl LocalMemoryStore {
        ...
        let mut hits = self
            ...
            .flat_map(|turn| {
                ...
                turn.retention_candidates
                    ...
                    .map(move |candidate| {
                        let title =
                            derive_candidate_title(candidate.profile.as_str(), &candidate.text);
    ...
    fn turn_path(&self, session_id: &str, turn_id: u64) -> PathBuf {
        if session_id.trim().is_empty() {
            ...
            self.turns_dir().join(format!(
                "session-{}-turn-{turn_id:020}.json",
                session_id.trim()
            ))
        }
    }
...
fn compact_archived_turn_record(record: &mut ArchivedTurnRecord) -> bool {
    ...
    for summary in &mut record.summaries {
        let compacted =
            compact_summary_text(&summary.summary, AGGRESSIVE_COMPACTED_SUMMARY_CHAR_LIMIT);
        if compacted != summary.summary {
...
fn update_observations(observations: &mut Vec<ProjectObservation>, record: &ArchivedTurnRecord) {
        ...
        if let Some(existing_index) = observations.iter().position(|observation| {
            ...
            if normalized_text(&existing.summary) == normalized_text(&candidate.text) {
                ...
                existing.evidence_refs =
                    merge_observation_evidence(&existing.evidence_refs, &evidence_refs);
                ...
                existing.corrected_by =
                    Some(observation_id(&record.session_id, record.turn_id, index));
    ...
    let completed_tasks = record
        ...
        .map(|task| {
            canonical_key(
                "open_threads",
                &derive_candidate_title("open_threads", &format!("Open thread: {task}")),
            )
        })
        .collect::<BTreeSet<_>>();
...
fn memory_query_score(title: &str, summary: &str, user_intent: &str, queries: &[String]) -> u32 {
    aggregate_query_scores(queries, |query| {
        field_weighted_match_score(query, &[(title, 14), (summary, 8), (user_intent, 4)])
    })
}
...
mod tests {
    use super::{
        ...
        MemoryQuery, ObservationFreshness, ObservationQuery, RetentionProfile, StepContextHint,
        StepSummary, SummaryPriority, TurnRetentionSignals,
    ...
    fn archive_turn_persists_retention_profiles_and_noise_gate() {
        ...
        assert!(record
            ...
            .iter()
            .any(
                |candidate| candidate.profile == RetentionProfile::ProjectFacts
                    && candidate.accepted
            ));
        assert!(record
            .retention_candidates
            .iter()
            .any(
                |candidate| candidate.profile == RetentionProfile::DeveloperPreferences
                    && candidate.accepted
            ));
        assert!(record
            .retention_candidates
            .iter()
            .any(
                |candidate| candidate.profile == RetentionProfile::OpenThreads
                    && candidate.accepted
            ));
        assert!(record
            .retention_candidates
        ...
        assert!(root
            .join(format!(
                ...
                omega_hpc_paths::MEMORY_TURNS_DIR_PATH
            ))
            .exists());
    }
        ...
        store
            .archive_turn(ArchivedTurnInput {
                ...
                summaries: vec![summary(
                    "feature",
                    "plan",
                    "Wire memory query into the planner",
                )],
                signals: TurnRetentionSignals {
                    changed_paths: vec!["crates/omega-context/src/lib.rs".to_string()],
    ...
    fn completed_open_thread_marks_previous_observation_superseded() {
        ...
        assert!(observations
            .iter()
            .any(|observation| { observation.freshness == ObservationFreshness::Superseded }));
    }
        ...
        store
            .archive_turn(ArchivedTurnInput {
                ...
                signals: TurnRetentionSignals {
                    ...
                    validation_targets: Vec::new(),
                    developer_preferences: vec![
                        "Prefer recall bundle naming in diagnostics".to_string()
                    ],
                    governance_events: Vec::new(),
                },
            })

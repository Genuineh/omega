...
use omega_hpc_document::{
    ...
    StructuredDocRelationRecord, StructuredDocsCutoverResult, StructuredDocsSnapshot,
    StructuredDocsValidationReport, StructuredDocumentRecord,
};
    ...
    I: IntoIterator<Item = String>,
{
    match run_inner(args.into_iter().collect()) {
        Ok(output) => output,
        Err(error) => CliOutput::new(classify_error(&error), String::new(), format!("{error}\n")),
    }
}
...
fn doctor_command(mut args: Vec<String>) -> Result<CliOutput> {
    ...
    if root_exists && docs_exists && docs_data_exists && manifest_exists {
        ...
        let validation = documents.manage_document(DocumentOp::ValidateProjection {
            doc_ids: Vec::new(),
        })?;
    ...
    let ok =
        root_exists && docs_exists && docs_data_exists && manifest_exists && version_issues == 0;
    let payload = json!({
        "command": "doctor",
...
fn cutover_command(mut args: Vec<String>) -> Result<CliOutput> {
    ...
    let documents = OmegaDocument::new(root.clone());
    let result =
        documents.run_structured_docs_cutover(DocumentMutationMode::Apply, args, doc_type)?;
        ...
        ListKind::Relations => {
            if json {
                return Ok(CliOutput::new(
                    SUCCESS,
                    to_pretty_json(
                        &json!({ "kind": "relations", "relations": snapshot.relations }),
                    ),
                    String::new(),
                ));
            }
...
fn nested_mutation_command(kind: EntityKind, mut args: Vec<String>) -> Result<CliOutput> {
    ...
    let mode = take_mode(&mut args, DocumentMutationMode::Apply)?;
    let input_path =
        take_option_flag(&mut args, "--input")?.ok_or_else(|| anyhow!("--input is required"))?;
...
fn remove_command(mut args: Vec<String>) -> Result<CliOutput> {
    ...
    let result = match entity_kind {
        EntityKind::Record => {
            documents.manage_document(DocumentOp::DeleteRecord { mode, doc_id: id })?
        }
...
fn take_mode(
    args: &mut Vec<String>,
    default: DocumentMutationMode,
) -> Result<DocumentMutationMode> {
...
fn render_cutover_output(
    ...
    if json {
        ...
        return CliOutput::new(
            if ok { SUCCESS } else { INVALID_INPUT },
            to_pretty_json(&payload),
            String::new(),
        );
    }

...
- `crates/`: Rust workspace crates (main omega workspace).
- `omega-hpc/`: Sibling sub-workspace that owns the memory + document + doc-cli +
  project-layout stack (`omega-hpc-paths`, `omega-hpc-memory`, `omega-hpc-document`,
  `omega-hpc-doc-cli`). The main workspace excludes `omega-hpc/` from
  `members` and depends on it via path deps to the re-prefixed crates. See
  `docs/specs/omega-hpc-extraction.md` for the extraction contract and
  `docs/decisions/007-omega-hpc-extraction.md` for the ADR.
...
Some files under `docs/` are auto-generated by scripts in `scripts/docs/`.
Hand-edits to these files are forbidden — change the source of truth
(record set, theme TOML, or script) and re-run the generator instead.

| Output file | Generated by | Source of truth |
|-------------|---------------|-----------------|
| `docs/decisions/README.md` | `scripts/docs/render-decisions-index.py` | `docs-data/records/decisions.jsonl` |
| `docs-data/relations/edges.jsonl` | `scripts/docs/build-relations.py` | markdown link scan of `docs/**/*.md` |
| `docs/TODO.md` Current Baseline section | `scripts/docs/render-todo-baseline.py` | `docs-data/records/decisions.jsonl` + `CHANGELOG.md` |

## CI / Lint Scripts

The repo's CI (`.github/workflows/doc-sync.yml`) runs these checks on
every PR. They are also safe to run locally:

- `scripts/docs/doc-sync.sh` — for every docs file with
  `last_verified_commit`, ensure no referenced source file has been
  modified after that commit. Reports `last_verified_commit: N/A` as
  a warning.
- `scripts/docs/spec-lint.sh` — for every spec, ensure the canonical
  sections (Overview / Goals / Non-Goals / Architecture / Data-Model /
  Testing / Change Log) are present, and the `language` field is set.
- `scripts/docs/backfill-frontmatter.sh` — one-shot script for filling
  in missing `last_verified_commit` / `source_path` / `language`
  fields. Re-runs are idempotent. For uncommitted files, falls back
  to a `uncommitted+<blob-hash>` placeholder so the field is never N/A.
- `scripts/docs/sync-records-from-fm.py` — bridges the on-disk
  backfill into the record set so the next render does not revert
  the values. Run after `backfill-frontmatter.sh`.
- `scripts/docs/force-fill-records-from-git.py` — for every record
  whose `frontmatter.last_verified_commit` is still `N/A`, look up
  the file's last commit SHA and write it to the record directly.
  Breaks the cascading-write cycle that previously reverted the
  backfill on the next render.

If a new doc needs to appear in `decisions/README.md`, run
`omega-doc extract <path> --apply` first, then re-run
`render-decisions-index.py`. Do not hand-edit the table — it will
silently drift from the record set.
